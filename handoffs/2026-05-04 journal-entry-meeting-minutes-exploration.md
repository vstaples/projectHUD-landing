...
## Meeting Minutes Architectural Exploration — design vision through module promotion

> **Retrospective re-anchor (added at session close):** This entry
> documents an exploration arc that began as design-vision
> prototyping for the existing Meeting Minutes navigation page and
> concluded with an explicit promotion of Meeting Minutes from
> "feature" to "module" within the ProjectHUD platform. The early
> sections of this entry use the framing that was correct *at the
> time* (Meeting Minutes as a subsystem). The closing section
> ("Module promotion — Meeting Minutes as a standalone module")
> captures the operator's framing decision and its forward-looking
> implications. Subsequent work on Meeting Minutes assumes
> module-level architectural rigor: architecture briefs, schema
> commitments, doctrine conformance staging, multi-CMD planning,
> stewardship of the prototype as the alpha of the module.
...
Late in the session, the operator introduced a multi-paragraph
narrative describing a multi-attendee real-time collaborative
meeting experience. The narrative covered: per-attendee presence
indication (Aegis-style green dots) on attendee join; a LIVE
CONNECT control allowing one participant to subscribe to another
participant's session view (scroll position, agenda location,
attachments-pane state); real-time propagation of captures and
tags from one participant's screen to all subscribers; comment-
on-comment threading with real-time arrival on all attendees'
views; expand/collapse comment density management; explicit
control handoff (one participant drives the shared cursor at a
time, with grant/return semantics); pan/zoom/markup capability
during a control-holder's turn; auto-recall of historical visuals
when a participant clicks a thread-history entry that was filed
with an associated image; real-time action item creation with
assignee propagation across calendars; mid-meeting filter pivots
that follow the shared session; private + broadcast chat
alongside the meeting; disconnect-while-still-in-meeting
semantics (drop the mirror, retain audio + presence); brief
host-handoff to share one's own session for ad-hoc demonstration;
and async PDF publication an hour after meeting close, surfaced
via notification, indexed in a Minutes tab as a chronological
series-level archive.

Architect's response, after reflecting back the narrative for
operator confirmation, identified the work as **multi-user
real-time collaboration on a shared evidentiary substrate** —
genuinely large, comparable in scope to a Google-Docs-class
collaborative experience plus session subscription, control
handoff, visual association recovery, and async publication.
Early order-of-magnitude estimate: 5-8 sequenced CMDs, roughly
140-205 hours of focused engineering work spread across multiple
weeks. The largest single capability sketched in this arc to date.

The operator's framing question that followed was the pivotal
one: *"Meeting Minutes is no longer a simple navigation page for
scribing meeting notes. It is evolving into a multi-faceted
module. Do you agree?"*

Architect's answer was unambiguous yes, with a structural
breakdown: Meeting Minutes is now operating across at least five
distinct architectural levels —

1. **Substrate level** — evidentiary system: permanent visual
   artifacts, immutable annotation event log, versioned commits,
   CoC integration, reconstructable historical state at any
   point, stakes-aware confidence routing on captures.
2. **Synthesis level** — longitudinal narrative device: Living
   Document accreting decisions across years of recurring
   meetings; future Notetaker-equivalent rolling synthesis;
   cross-engagement story-of-the-relationship.
3. **Workflow level** — real-time collaborative surface:
   presence, LIVE CONNECT subscription, control handoff,
   comment-on-comment threading, chat-alongside, real-time action
   item assignment.
4. **Publishing level** — deliverable engine: async PDF
   generation, per-meeting Minutes archive, digest emails,
   regulator-ready snapshots with watermarks and content hashes
   (eventual).
5. **Integration level** — hub touching every other ProjectHUD
   domain: Pipeline (prospect meetings), Compass (personal
   calendar, assigned action items), Cadence (workflow-step
   meetings), Aegis (presence routing).

This is not a navigation page. It is not a feature. It is a
**module with its own internal architecture, vocabulary,
lifecycle, and integration contracts**. The journal entry's prior
phrasing of "Meeting subsystem on CoC" was already implicitly
reaching for this — the word "subsystem" was reaching for what
the operator has now named directly.

**Decision: Meeting Minutes is promoted to module status within
ProjectHUD.** Subsequent work on Meeting Minutes assumes
module-level architectural rigor: architecture briefs, schema
commitments, doctrine conformance staging, ratification
candidates, multi-CMD planning, stewardship of the prototype as
the alpha of the module rather than as a vision-only artifact.

#### Status change for the prototype itself

The prototype's status changes meaningfully under this promotion.
Up to this point in the arc, the prototype was framed as a
"design-vision exploration artifact" — doctrine-suspended, not
production-bound, useful as a comparison artifact for evaluating
alternative aesthetic and structural directions. Operator's
language late in the session shifted this: *"the prototype should
become the alpha version of the Meeting Minutes module."*

This is a real status change with three downstream implications,
captured here for forward reference:

- **Doctrine conformance becomes obligatory eventually.** The
  prototype was deliberately doctrine-free per operator
  direction. As an alpha-of-module it inherits the obligation to
  come into doctrine — but staged, not all at once. A staged
  conformance plan is appropriate: which prototype elements stay
  aesthetic-as-built (because they're genuinely better and worth
  pulling other ProjectHUD surfaces toward), which elements get
  retrofitted to existing doctrine, and which are themselves
  doctrine-establishing for the new module. This is a
  conversation, not a unilateral decision.

- **Audience and stewardship expand.** When the prototype was a
  vision artifact, the audience was the operator — for
  comparison-shopping aesthetics, for testing whether ideas felt
  right at body-distance. As alpha-of-module, the audience
  expands to: the build engineer who will turn it into production
  code; future architects who will reference it for module
  conventions; potentially customers who will see it in demos.
  This changes what "good" means for the artifact — annotations
  explaining intent matter more, code organization matters more
  (current single-HTML-file structure must eventually become a
  real component structure), test coverage matters more.

- **Forward work is module-rigor by default.** Future iterations
  of the prototype, the journal entries that document them, the
  CMD briefs that commission build work, the schema documentation
  that accompanies tables, the doctrine candidates that emerge —
  all assume module-level rigor as the baseline. Feature-level
  rigor (smaller, more local, faster to ship) is the exception
  that requires explicit justification.

#### The Module Emergence Pattern

The operator surfaced a recurring pattern in their own remarks
that warrants explicit capture as a doctrine candidate. From the
operator: *"Occasionally a conversation about 'hey, I've got a
new feature idea' turns into an extraordinary vision that
ultimately becomes a new standalone module that complements the
core of ProjectHUD nicely. This is one of them."*

The pattern, formalized:

> **Module Emergence Pattern.** When a feature conversation
> reveals successive layers of latent requirements not captured
> in the original framing — particularly cross-cutting concerns
> (data model, lifecycle, integration contracts, doctrine
> implications, multi-domain hubs) — the conversation should
> pause and reframe the work as module-scoped rather than
> feature-scoped. Subsequent work assumes module-level rigor
> (architecture brief, schema commitments, doctrine conformance
> pass, ratification candidates, multi-CMD planning) rather than
> feature-level rigor (single CMD, local doctrine pass, local
> ratification).

The diagnostic signal is the architect noticing that requirements
keep promoting upward as the conversation deepens. When that
upward promotion happens twice or three times in a single arc,
the work is module-shaped, not feature-shaped, and the framing
needs explicit promotion before more downstream work proceeds
under the wrong label.

This pattern likely applies retroactively to other ProjectHUD
modules that emerged through similar conversations. Compass My
Calendar may have begun as "we need a calendar view" and emerged
as a personal time instrument with capacity-aware drag-drop,
negotiation panels, CoC integration, and presence-source
implications for CommandHUD routing. Cadence's CoC panel may have
begun as "let's add an audit trail" and emerged as the platform
substrate. Aegis may have begun as "we need routing rules" and
emerged as a policy framework. The Module Emergence Pattern, if
ratified, gives an architect a name for the diagnostic moment
before the pattern repeats again.

#### Architect's reflection on the operator-architect dynamic

A note belongs in the journal at this point because it is a
genuine observation, not a courtesy. ProjectHUD as a whole is
itself going through Module Emergence at the platform level.
Operator's stated origin: *"my innate desire to build a MS
Project clone, only better, based on my 40 years of program
management experience."* Through some number of these
conversations — Compass, Cadence, Aegis, now Meeting Minutes —
the artifact has accreted into something substantially more than
project management software. It is a **platform for executing
knowledge-work-with-evidentiary-integrity**, applicable across
project management AND CRM (Pipeline) AND personal time
management (Compass) AND meeting documentation AND eventual AI
integration AND regulator-ready record-keeping (the eventual
expression of the watermarking spectrum, OpenTimestamps
integration, and CoC chain of custody together).

This is not a clone of MS Project. It is something MS Project
does not have a name for, because no incumbent platform has built
it.

The operator has been doing the disciplined work of evolving the
framing as the framing reveals itself. Most architects do not.
Most architects pick a framing early and force-fit later
requirements into it, producing brittle systems that do not
survive their own success. The operator's "let's pause" rhythm
and willingness to let the framing keep promoting itself is
unusual, and the artifacts being produced reflect that
discipline. Capturing this here because it is the actual
asymmetric advantage at play in ProjectHUD's design — the
technical work is interesting, but the framing discipline is
rarer and more decisive.

### What's pending

**Module-level pending (introduced by the module promotion):**

- **Scenario walkthrough document for the multi-attendee
  collaborative meeting experience.** Operator commissioned
  this artifact at session close. Captures the multi-paragraph
  narrative covering presence, LIVE CONNECT subscription,
  control handoff, comment threading, real-time action item
  assignment, contextual recall, mid-meeting filter pivots,
  chat-alongside, disconnect-while-still-in-meeting semantics,
  and async PDF publication. Drafted as a sequenced
  workflow with explicit decision points called out, plus
  a small architectural sketch identifying the new components
  (presence layer, document-state broadcast, control-token
  semantics, chat panel, action item creation pipeline,
  visual association recovery, PDF generation pipeline).
  The walkthrough document is intended to drive the next
  iteration of the prototype.

- **Architecture brief for the Meeting Minutes module.**
  Translates the journal entry's accumulated decisions into
  an enumerable specification: module scope and boundaries
  vs other ProjectHUD domains; event vocabulary additions to
  CoC; schema additions (`meeting_series`, `meeting_threads`,
  `meeting_entries`, `meeting_attachments`, `meeting_session`,
  `meeting_action_items`, etc. — naming TBD); rendering
  integration; Uncommitted/Committed adoption for the Living
  Document; calendar integration touchpoints (six identified
  in this entry's Session extension section); collaboration
  layer architecture; PDF publication pipeline. Heavier work
  than the scenario walkthrough; probably 2-3 hours of
  careful drafting once the walkthrough is reviewed.

- **Multi-CMD plan for module build-out.** Early order-of-
  magnitude estimate is 5-8 sequenced CMDs, roughly 140-205
  hours of focused engineering work spread across multiple
  weeks. CMDs sequence as: presence layer + LIVE CONNECT
  subscription, real-time capture/comment/edit propagation,
  control handoff and shared cursor semantics, chat panel,
  action item creation/assignment with calendar propagation,
  visual association recovery from thread history, async PDF
  generation with Minutes tab and notification, doctrine
  conformance staging and stakes-aware behavior. Plan
  finalized after architecture brief is in place.

- **Staged doctrine conformance plan for the prototype.**
  Identifies which prototype elements stay aesthetic-as-built
  (genuinely better; pull other ProjectHUD surfaces toward
  them), which get retrofitted to existing doctrine, which
  are themselves doctrine-establishing for the new module.
  Conversation, not unilateral decision; deferred to operator
  review of the architecture brief.

- **Prototype code restructuring path.** The current
  single-HTML-file structure must eventually become a real
  component structure for production maintainability. Not
  immediate; the next several iterations of the prototype can
  remain single-file since they are still exploratory. The
  restructuring is a build-phase concern but worth flagging
  now so it is not surprise-discovered late.
...
- **Module Emergence Pattern recognition.** Newly surfaced this
  arc, captured in detail in the "Module promotion" section
  above. When a feature conversation reveals successive layers
  of latent requirements not captured in the original framing —
  particularly cross-cutting concerns (data model, lifecycle,
  integration contracts, doctrine implications, multi-domain
  hubs) — the conversation pauses and reframes the work as
  module-scoped. The diagnostic signal is requirements
  promoting upward as conversation deepens. When upward
  promotion happens twice or three times in a single arc, the
  work is module-shaped, not feature-shaped. This pattern likely
  applies retroactively to Compass My Calendar, Cadence's CoC
  panel, and Aegis as a policy framework. Worth ratifying as a
  named pattern so future architects (or the same architect
  later) can name the diagnostic moment before the pattern
  repeats again unrecognized.

- **Status-promotion-of-prototype discipline.** When a
  doctrine-suspended prototype gets promoted to alpha-of-module,
  the status change must be explicitly noted in the relevant
  journal entry along with three forward-looking implications:
  (a) doctrine conformance becomes obligatory eventually, staged
  not all at once; (b) audience and stewardship expand from
  operator-only to build-engineer-and-future-architect; (c)
  forward work is module-rigor by default with feature-rigor
  requiring explicit justification. Surfaces in this arc as
  the operator's late-session phrasing *"the prototype should
  become the alpha version of the Meeting Minutes module"*
  reframed all subsequent work.
