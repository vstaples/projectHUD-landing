# ProjectHUD Style Doctrine v1.8
...
amended 2026-04-28 (v1.7); amended 2026-05-08 (v1.8)
**Authority:** operator (product) + architect (code) — see §0.1.
v1.8 ratified during CMD-ACCORD-CONSTELLATION-ENTRY-1 Phase 4b
following 3 cross-phase data points (Phase 2 IR64 finding;
Phase 3 tree adaptation; Phase 4a views adaptation). Future
revisions require both signatures.
...
**v1.8 changelog:**
- **§3.8 added** (module-palette discipline): when a module's
  visual treatment is adapted from another module's pattern
  (Compass tree → Accord tree, Pipeline drag-drop → Accord
  drag-drop, etc.), tokens borrowed from the inspiration-source
  module must be swapped to the consuming module's tokens before
  commit. Three cross-phase data points within
  CMD-ACCORD-CONSTELLATION-ENTRY-1 (Phase 2 cyan→amber palette
  swap in constellation; Phase 3 cyan→amber palette swap in
  tree; Phase 4a cyan→amber palette swap in views). Pattern is
  ratified as Style Doctrine clause; promotion to Iron Rule
  waits for the pattern surviving under different
  inspiration-source pressure (i.e., a non-Compass borrow). See
  §3.8 for the full rule.
...
### §3.8 Module-palette discipline (v1.8)

When a module's visual treatment is **adapted from another
module's pattern** — Compass tree adopted into Accord left rail,
Pipeline drag-drop pattern adopted into Accord parking-lot
flows, Cadence form patterns adopted elsewhere, etc. — every
color/palette token borrowed from the inspiration-source module
must be swapped to the consuming module's tokens before commit.

**The rule.** Adaptation reuses *vocabulary, structure, and
behavior*. It does NOT reuse *visual identity*. Each module
owns a palette; that palette signals "you are in module X" to
the operator. A borrowed token from a sister module signals "you
are in the wrong module" — it breaks the operator's spatial
sense of which surface they're on.

**Application.**
1. When opening a file from another module as inspiration,
   identify every token reference (`--cyan`, `--text-accent`,
   bare hex like `#00D2FF`) that carries the inspiration
   module's identity.
2. Map each to the consuming module's equivalent (Compass cyan
   → Accord amber; Pipeline accent → consuming module's accent;
   etc.).
3. Verify the swap is complete before commit. A grep for the
   inspiration module's distinctive hex values should return
   zero matches in the consuming module's files.

**What this rule does NOT govern.**
- Cross-module utility tokens (`--surface-base`, `--text`,
  `--rule`, etc.) that are platform-shared rather than
  module-identifying. These propagate freely.
- State-color tokens (§3.6) which are platform-wide, not
  module-specific.
- Functional patterns (drag-drop mechanics, modal structure,
  keyboard handlers). The rule governs visual identity only.

**Pre-doctrine examples (recorded for posterity).** Three
cross-phase swaps within CMD-ACCORD-CONSTELLATION-ENTRY-1
established the pattern:
- **Phase 2** — scaffolding §3.1 specified "active workstreams
  cyan/teal" for constellation node glow; Accord palette is
  editorial-amber. Implementation swapped scaffolded cyan
  references to `--signal` (#d4a04a). IR64 finding.
- **Phase 3** — left-rail tree adapted from `my-meetings.html`
  Knowledge Tree (Compass cyan throughout `.mtg-*` classes).
  `.ac-tree-*` rewrite swapped every cyan reference to Accord
  amber. Vocabulary (chevron, `.active` highlight, outcome
  dots) reused; identity not.
- **Phase 4a** — `accord-views.css` shipped without any cyan
  borrows. Pattern was already internalized by this point —
  the discipline propagates forward through coding agents who
  see the pattern hold across phases.

**Promotion to Iron Rule.** This clause stays Style Doctrine
until the pattern survives under **different inspiration-source
pressure**. Three data points all point at the same root cause
(Compass-borrowed cyan during Accord work). Iron Rule
promotion waits for a non-Compass borrow — likely
CMD-ACCORD-NRA-SUBSTRATE-1 or later — proving the discipline
generalizes beyond a single vendor-of-borrows.
