...
## Version 1.1 · Authored 2026-05-09 · Updated 2026-05-09
...
**Numbering system:** Each CMD carries a track prefix and sequence number per the master CMD registry (see companion document). Track C = Accord Meeting Setup shell. Format: `C-NN · CMD-NAME`.

### Wave 1 — Foundation wiring (Track C, CMDs 1–5)

**Goal:** Setup shell looks like mockup v5. Structurally correct. Operator recognizes it immediately.

| # | CMD | Status | Scope |
|---|---|---|---|
| C-01 | `CMD-ACCORD-SETUP-LAYOUT-1` | **SEALED** | 5-zone CSS grid; full-page host; drag-resize columns; mockup v5 token palette; no content yet |
| C-02 | `CMD-ACCORD-SETUP-HEADER-1` | **IN PROGRESS** | Meeting title, Stakes, location, schedule meta, FOLLOW-UP/FIRST-EVER toggle; `stakes` + `location` migration |
| C-03 | `CMD-ACCORD-SETUP-OUTCOMES-1` | QUEUED | `accord_meeting_outcomes` substrate + center column outcomes block above agenda |
| C-04 | `CMD-ACCORD-SETUP-ATTENDEES-1` | QUEUED | `accord_meeting_attendees` substrate + right column roster (public-safe cards only) |
| C-05 | `CMD-ACCORD-SETUP-FILMSTRIP-2` | QUEUED | Full-width filmstrip zone with drag-resize, 3 density states, scrub-to-Minute-Notes |

### Wave 2 — Intelligence and depth (Track C, CMDs 6–10)

**Goal:** Setup shell behaves like mockup v5. All panels populated. Intelligence Mode operational.

| # | CMD | Status | Scope |
|---|---|---|---|
| C-06 | `CMD-ACCORD-SETUP-BRIEFING-TABS-1` | QUEUED | Left column tabs (Briefing/Decisions/Risks); enhanced briefing content; Risk Register consumption |
| C-07 | `CMD-ACCORD-SETUP-AGENDA-ENHANCED-1` | QUEUED | Agenda item types; time estimates; prep prompt; drag-to-reorder; `item_type` + `duration_minutes_estimate` migration |
| C-08 | `CMD-ACCORD-SETUP-INTELLIGENCE-1` | QUEUED | Cmd+I overlay; private layer; `accord_meeting_intel_notes` substrate; substrate-derived pattern data |
| C-09 | `CMD-ACCORD-SETUP-ACTION-KANBAN-1` | QUEUED | Action Items time-anchored kanban + calendar view in right column tab. **Note: CPM overlay (slack indicators, critical path spine) requires Track F CMDs first; v1 ships on `due_date` only.** |
| C-10 | `CMD-ACCORD-SETUP-SLIDESHOW-1` | QUEUED | Auto-rotation; stepper; freeze-on-hover; progress indicator; asymmetric column policies |

### Wave 3 — Refinement and close (Track C, CMDs 11–13)

**Goal:** Full interaction layer. Every element alive. Product matches + exceeds mockup.

| # | CMD | Status | Scope |
|---|---|---|---|
| C-11 | `CMD-ACCORD-SETUP-PERCOLATE-1` | QUEUED | Click-to-percolate-by-person across all panels |
| C-12 | `CMD-ACCORD-SETUP-GATHERING-1` | QUEUED | Gathering mode auto-engage; 5-min warning; temporal state transitions |
| C-13 | `CMD-ACCORD-SETUP-VERDICT-1` | QUEUED | Footer verdict pill with readiness computation; budget bar with item estimates; warning pills |

### Post-Wave 3 — Ancillary and AI upgrades (Track X)

| # | CMD | Status | Scope |
|---|---|---|---|
| X-01 | `CMD-ACCORD-MEETING-ATTACHMENTS-1` | QUEUED | Carried References, file attachments |
| X-02 | `CMD-ACCORD-MEETING-INVITATIONS-1` | QUEUED | Save & invite flow |
| X-03 | `CMD-ACCORD-OUTCOME-RATIFICATION-1` | QUEUED | Post-meeting outcome marking; workstream achievement rate metrics |
| X-08 | `CMD-BRIEFING-SYNTHESIS-1` | QUEUED | AI-powered briefing synthesis via Anthropic API; upgrades `briefing_text` derivation layer |
| X-09 | `CMD-ATTENDEE-PATTERNS-1` | QUEUED | AI-powered per-attendee pattern inference; upgrades Intelligence Mode derivation layer |
| — | CPM/PERT Risks tab consumption | QUEUED | Requires Track F (CMD-CPM-DERIVED) to ship first |
| — | Sentiment integration | QUEUED | Behavioral pattern expression only; no raw sentiment scores |

### Prerequisite tracks (must complete before some Wave 2/3 features activate)

| Track | CMDs | Gates |
|---|---|---|
| Track D — Projection Engine + Counterfactual | D-01, D-02, D-03 | Morning brief; executive briefing; counterfactual preview in Intelligence Mode |
| Track E — Cross-module Integration | E-01 (Compass Bridge), E-02 (CPM substrate) | Action item status sync; CPM input data |
| Track F — CPM and PERT | F-01 (CPM-DERIVED), F-02 (PERT) | C-09 CPM overlay; Risks tab CPM surface; PERT forecasts |
| Track G — Resource and Schedule | G-01, G-02 | Resource heatmap; schedule manipulation |
| Track H — Daily Experience | H-01, H-02, H-03 | Morning brief; living reference; identity unification |
...
- v1.1 — 2026-05-09 — §14 wave plan updated with track/sequence numbering system (C-01 through C-13, X-01 through X-09); prerequisite track dependency table added; C-09 CPM overlay prerequisite note added; status column added to wave tables.
