...
- **Existing CMD-AEGIS-VERIFICATION-PATTERN** (the convention CMD that establishes Aegis-runnable verification flows as standard hand-off artifact, plus the `Wait ForConsole` verb addition) — held earlier in the build arc behind the auth-init-race fix, currently unshipped. Its reference exemplar scripts will be authored AFTER this CMD ships, directly into the new substrate as published playbooks. No exemplars are migrated as part of this CMD because none exist yet.
...
### §9.3 Library UI — basic interaction
...
### §9.4 Lifecycle transitions (DOCTRINAL FLOOR)
...
### §9.8 Backward compatibility — `Run` verb
...
### §9.9 Backward compatibility — `Register` verb
...
### §9.11 IR45 vocabulary preserved
...
If §9.2 (migration) or §9.4 (lifecycle transitions) fails, halt and surface — those are the primary deliverables.
...
§9 specifies eleven behavioral verification subtests. §9.2
(migration) and §9.4 (lifecycle transitions) are the doctrinal-
floor checks.

Hand-off format per §11.

Halt on missing input. Halt if §9.2 or §9.4 fails.
