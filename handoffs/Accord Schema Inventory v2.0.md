...
## §15 — Session 2026-05-13/14 final state

### Surface state class fix — `accord-views.js`

Two code paths now correctly stamp `.ac-center` with meeting state class:

**Path 1 (line 409) — initial render:**
```javascript
_sfCenter1.classList.add('meeting-' + (meeting.state || 'idle'));
```

**Path 2 (line 517) — re-render on navigation:**
```javascript
...
**CSS dependency:** `.ac-meeting-surface-host.active .ac-setup-shell { display:none }`
and `.ac-center.meeting-idle #ac-live-filmstrip { display:none }` in `accord-views.css`.

### `accord-slideshow.js` race condition note

The slideshow module uses dynamic `load()` in `accord.html` — no guaranteed
execution order vs `accord-rails.js`. Guards in `accord-rails.js` use
`typeof window.AccordSlideshow.dismiss === 'function'` before calling.
If `AccordSlideshow` is empty on load, a 500ms defer in `_renderTree` would
resolve — deferred to follow-on if race condition recurs.

### New files in `/js/`

| File | Purpose |
|---|---|
| `accord-slideshow.js` | Constellation onboarding slideshow |
| `accord-my-meetings.js` | MY MEETINGS tabbed rail dashboard |

### Iron Rules in effect (full list for next architect)

| IR | Summary |
|---|---|
| IR41 | Composer is local; only commit gestures broadcast |
| IR42 | Post-seal mutations rejected at DB |
| IR65 | Version bump operator-managed (`version.js`) |
| IR66 | Console-first debugging |
| IR67 | File header version + date on every modified file |
| IR68 | One diagnostic at a time |
| IR69 | Test instructions proactively, one step at a time |
| IR70 | Minimum font sizes: rail 10px, card title 12px, meta 10px, mono 9px, btn 10px |
...
| v2.0 | 2026-05-14 | Added §15: surface state class fix paths, slideshow race condition note, new files, full IR list for handoff. |
