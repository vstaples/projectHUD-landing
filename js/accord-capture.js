...
(() => {
  ...
  // Verbatim port of Setup shell filmstrip (C-05 + C-14).
  // State vars required by the copied functions:
  var _filmstripAborted    = false;
  var _filmstripToken      = 0;
  ...
  // Mount: inject filmstrip zone HTML (same structure as Setup shell _buildHTML)
  function _mountLiveFilmstrip(meeting) {
    ...
    if (captureBody) captureBody.insertAdjacentElement('afterend', zone);
    else surface.appendChild(zone);

    _wireFilmstripHandle();
    _renderFilmstrip(meeting, meeting.workstream_id);
  }

  // Verbatim from accord-meeting-setup.js _renderFilmstrip:
  function _renderFilmstrip(meeting, workstreamId) {
    ...
    _fetchFilmMeetings(meeting.meeting_id, workstreamId)
      ...
      .catch(function(e) {
        console.error('[AccordCapture] filmstrip fetch failed', e);
        if (_filmstripToken !== myToken) return;
        if (!content.isConnected) return;
  ...
  // Verbatim from accord-meeting-setup.js _fetchFilmMeetings:
  ...
  // Verbatim from accord-meeting-setup.js _fetchFilmNodeCounts:
  ...
  // Verbatim from accord-meeting-setup.js _paintFilmstrip:
  ...
  // Verbatim from accord-meeting-setup.js _filmDate:
  ...
  // Verbatim from accord-meeting-setup.js _filmCountSummary:
  ...
  // Verbatim from accord-meeting-setup.js _wireFilmstripEvents —
  // ONLY CHANGE: replace _activateScrub with _onLiveFilmSelect (Step 5):
  function _wireFilmstripEvents(content, meetings, currentMeeting, workstreamId) {
    ...
    if (track) {
      track.addEventListener('click', function(ev) {
        ...
        if (meetingId === currentMeeting.meeting_id) return;
        ...
        // ONLY CHANGE FROM SETUP SHELL: Thread History filter instead of scrub overlay
  ...
  // Verbatim from accord-meeting-setup.js _scrollToCurrentFrame:
  ...
  // Verbatim from accord-meeting-setup.js _initFilmDensity:
  ...
  // Verbatim from accord-meeting-setup.js _enrichFilmCards:
  function _enrichFilmCards() {
    ...
    API.get(
      ...
      console.error('[AccordCapture] filmcard enrich failed', e);
    });
  }

  // Verbatim from accord-meeting-setup.js _paintFilmCardContent:
  ...
  // Verbatim from accord-meeting-setup.js _initFilmCardTiers:
  ...
  // Verbatim from accord-meeting-setup.js _stopFilmCardTiers:
  ...
  // Verbatim from accord-meeting-setup.js _isSameDay / _fmtShort / _getWeekBounds:
  ...
  // Resize handle (same logic as Setup shell _wireFilmstripHandle / _onFilmHandleMouseDown):
  function _wireFilmstripHandle() {
    var handle = document.querySelector('#ac-live-filmstrip .ac-filmstrip-handle');
    if (!handle) return;

    handle.addEventListener('mousedown', function(ev) {
      ev.preventDefault();
      var zone   = document.getElementById('ac-live-filmstrip');
      var startY = ev.clientY;
      var startH = zone ? zone.offsetHeight : 100;
  ...
  // Thread History filter (live-mode exclusive — replaces scrub overlay):
  function _onLiveFilmSelect(meetingId, meetings) {
    var track = document.getElementById('ac-film-track');
    ...
    _liveActivateThreadHistoryTab();
    _liveLoadFilmThreadHistory(meetingId, meetings);
  }

  function _liveFilmClearFilter() {
    _liveFilmActiveId = null;
    var track = document.getElementById('ac-film-track');
    if (track) track.querySelectorAll('.ac-film-frame--active').forEach(function(f) {
      f.classList.remove('ac-film-frame--active');
  ...
  function _liveActivateThreadHistoryTab() {
  ...
  function _liveRestoreCaptureStream() {
  ...
  function _liveLoadFilmThreadHistory(meetingId, meetings) {
