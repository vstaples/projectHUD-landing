// VERSION: 20260418-CMD57
window._mwCoreVersion = 'v20260418-CMD57';
console.log('%c[mw-core] v20260418-CMD57 — B1 event bus: location.ready emit','background:#c47d18;color:#000;font-weight:700;padding:2px 8px;border-radius:3px');
...
window._mwLoadUserView = async function() {
  // Re-entrancy guard — prevent concurrent calls from racing on the loading screen
  if (window._mwLoadUserViewRunning) {
    console.warn('[mwLoadUserView] already running — skipped re-entrant call');
    return;
  }
  window._mwLoadUserViewRunning = true;
  console.log('[mwLoadUserView] entered | resource:', _myResource?.id?.slice(0,8)||'NONE');
  ...
  if (!_myResource?.id) {
    ...
    window._mwLoadUserViewRunning = false;
    return;
  }
...
// ── Recommended daily sequence panel ─────────────────
    ...
    window._mwLoadUserViewRunning = false;
    console.log('[mwLoadUserView] released guard');
  }
}